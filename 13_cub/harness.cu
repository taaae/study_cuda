// Exercise 13 harness — DO NOT EDIT.
...
#define SOLUTION_FILE "scan_cub.cu"
#endif
...
int main() {
    ...
    const int n = 1 << 24;                  // 16M ints
    const size_t bytes = (size_t)n * sizeof(int);

    std::vector<int> hin(n), hout(n), ref(n);
    for (int i = 0; i < n; ++i) hin[i] = (i * 1103515245 + 12345) & 0x1f;  // small ints

    // CPU exclusive scan reference (int sums stay in range with small inputs).
    long long acc = 0;
    ...
    // read input once, write output once -> ~2*bytes moved.
